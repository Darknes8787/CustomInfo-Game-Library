...
_Generated: 2025-10-09T17:48:01.953980Z_
...
* **Applicazione in tempo reale**: `set_setting("ui.theme", "light")` → emette `settings/changed` → la UI aggiorna `data-theme` e ricarica le variabili CSS.
...
1. Cambiare l'ombra dei giochi nella griglia quando si passa sopra con il mouse da nero a viola 
...
### F:/Programmi/Programmazione/Tool Libreria Giochi PC/disinstalla dipendenze.py
<a name="F:ProgrammiProgrammazioneTool Libreria Giochi PCdisinstalla dipendenzepy"></a>

```python
# -*- coding: utf-8 -*-
"""
CustomInfo — Uninstaller di dipendenze per test
- Legge requirements.txt alla root (o --req PATH)
- Disinstalla in blocco i pacchetti elencati (solo i nomi)
Uso:
    python uninstall_deps.py [--req PATH]
"""
...
def main():
    ...
    ap.add_argument("--req", type=str, default=None, help="Percorso a requirements.txt (default: auto in cwd o parent)")
    args = ap.parse_args()
    ...
    if not req_path:
        for c in [start/"requirements.txt", start.parent/"requirements.txt"]:
            if c.exists(): req_path = c; break

    if not req_path or not req_path.exists():
        print("[ERRORE] requirements.txt non trovato. Usa --req PATH")
        return 1

    names = parse_requirements_names(req_path.read_text(encoding="utf-8"))
    if not names:
        print("[INFO] Nessun pacchetto da disinstallare (requirements vuoto).")
        return 0

    print(f"[INFO] Disinstallazione pacchetti ({len(names)}): {', '.join(names)}")
